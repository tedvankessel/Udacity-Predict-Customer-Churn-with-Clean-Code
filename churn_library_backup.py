...
def perform_eda(df):
    '''
    perform EDA on df and saves figures to images folder
    input:
            df: pandas dataframe
...
def encoder_helper(df, category_lst, response):
    ...
    for column in category_lst:
        unique_categories = df[column].unique()  # Get unique categories in the column
        category_mapping[column] = {category: idx + 1 for idx, category in enumerate(unique_categories)}

    # Apply label encoding to each categorical column
    logging.info("INFO: apply mapping to features, reanme and drop old column")
    for column in category_lst:
        df[f'{column}_{response}'] = df[column].map(category_mapping[column])
...
def perform_feature_engineering(dataframe, response='Churn'):
    ...
    logging.info(f"INFO: {cat_columns}")
...
def classification_report_image(y_train,
    ...
    for model_index in range(len(model_name)):
        ...
        plt.text(0.01, 1.25, f'{model_name[model_index]} Train', fontsize=10, fontproperties='monospace')
        plt.text(0.01, 0.05, str(classification_report(y_train, y_train_models[model_index])), fontsize=10, fontproperties='monospace')
        plt.text(0.01, 0.6, f'{model_name[model_index]} Test', fontsize=10, fontproperties='monospace')
        plt.text(0.01, 0.7, str(classification_report(y_test, y_test_models[model_index])), fontsize=10, fontproperties='monospace')
...
def main():
    '''
    Runs all the functions above, inputting data from the DATA_PATH and processes it to the respective 
