...
Purpose: Test Module for the churn_library.py script
...
def test_import_data():
    ...
    try:
        ...
        logging.error("ERROR: The data doesn't have rows and columns")
        raise err
...
def test_perform_eda():
    ...
    for file_path in file_paths:
        try:
            ...
            logging.info(f"INFO: pre-existing file {file_path}: {os_error}")
    ...
    try:
        ...
        logging.error(f'ERROR: perform_eda fail {err.args[0]}')
        raise err
...
def test_encoder_helper():
    ...
    # in order to test the encoder_helper function we need to create
    # the necessary components to pass in
    ...
    try:
        ...
        logging.info('SUCCESS: encoder_helper makes the correct modification to the dataframe')
    except AssertionError as err:
        logging.error(
            "ERROR: encoder_helper did not make the correct modification to the dataframe")
        raise err

def test_perform_feature_engineering():
    '''
    test the perform_feature_engineering
    '''
    ...
    try:
        X_train, X_test, y_train, y_test = \
            cls.perform_feature_engineering(dataframe, response='Churn')
        logging.info("SUCCESS: perform_feature_engineeringr ran")
    except KeyError as err:
        logging.error('ERROR: perform_feature_engineering fail {err.args[0]}')
        raise err
...
def test_train_models():
    ...
    try:
        ...
        logging.error(f'ERROR: train_models fail {err.args[0]}')
        raise err
